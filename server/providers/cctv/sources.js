...
export async function loadTxdotSanAntonioSources() {
  ...
  try {
    ...
    for (const row of rows) {
      ...
      const netId = String(row?.netId || '').trim().toUpperCase();
      ...
      const direction =
        row?.dirDescription || row?.equipLoc?.direction || '';
      ...
      const cameraId =
        `txdot-sat-${slug}-` + hashSeed(icdId).toString(16);
    ...
    const maxRaw = Number(
      process.env.CCTV_TXDOT_SAT_MAX_SOURCES ||
        DEFAULT_TXDOT_SAT_MAX_SOURCES,
    );
